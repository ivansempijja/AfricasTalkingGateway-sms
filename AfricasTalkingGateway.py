"""
 ...
 THIS CLASS WAS REVISED BY IVAN DAVID SEMPIJJA TO ENABLE PYTHON SUPPORT FOR PYTHON 3
"""
    ...
    def sendRequest(self, urlString, data_ = None, authToken_ = None):
        try:
            ...
            if data_ is not None:
                ...
                request = urllib.request.Request(urlString, data.encode("utf-8"), headers = headers)
            else:
            ...
            response          = (b''.join(response.readlines())).decode("utf-8", "strict")
            if self.Debug:
